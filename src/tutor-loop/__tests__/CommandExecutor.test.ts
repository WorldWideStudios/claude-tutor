...
describe('CommandExecutor', () => {
  ...
  let mockUpdateProgress: jest.Mock<(workingDir: string, updates: any) => Promise<Progress>>;
  let mockAddCompletedStep: jest.Mock<(workingDir: string, step: string) => Promise<Progress>>;
  ...
  beforeEach(() => {
    mockUpdateProgress = jest.fn<(workingDir: string, updates: any) => Promise<Progress>>().mockResolvedValue(mockProgress);
    mockAddCompletedStep = jest.fn<(workingDir: string, step: string) => Promise<Progress>>().mockResolvedValue(mockProgress);
    // Use current directory for tests since test directory doesn't exist
    executor = new CommandExecutor(process.cwd(), mockUpdateProgress, mockAddCompletedStep);
  });
  ...
  describe('startsHeredoc', () => {
    ...
    it('should handle different delimiters', () => {
      ...
      expect(executor.startsHeredoc('cat > file << MARKER').delimiter).toBe('MARKER');
    });
  ...
  describe('executeCommand', () => {
    ...
    it('should update progress after file creation', async () => {
      const result = await executor.executeAndTrack('cat > test.txt << EOF\ntest\nEOF');
  ...
  describe('getMessageForAgent', () => {
    it('should format successful command message', () => {
      const message = executor.getMessageForAgent(
        'mkdir src',
        { success: true, output: '' }
      );

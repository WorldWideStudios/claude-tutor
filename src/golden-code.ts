...
export function parseGoldenCode(goldenCode: string): ParsedGoldenCode {
  ...
  while (i < lines.length) {
    ...
    const isCommand = commandPrefixes.some(
      (prefix) => line.startsWith(prefix + " ") || line === prefix,
    );
    ...
    while (i < lines.length) {
      ...
      const isNextCommand = commandPrefixes.some(
        (prefix) =>
          trimmedLine.startsWith(prefix + " ") || trimmedLine === prefix,
      );
    ...
    if (codeBlockLines.length > 0) {
      if (codeBlockLines.length === 1) {
        ...
        const blockComment = firstNonEmptyLine
          ? generateCodeComment(firstNonEmptyLine.code)
          : "";
        steps.push({
          type: "code-block",
...
function generateCodeComment(code: string): string {
  ...
  if (trimmed.startsWith("#!")) {
    ...
    if (trimmed.includes("python"))
      return "shebang - tells shell to use Python";
 ...
 */
export function hasMoreGoldenSteps(
  goldenCode: string,
  currentIndex: number,
): boolean {
  const parsed = parseGoldenCode(goldenCode);
